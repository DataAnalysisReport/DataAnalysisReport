 ...
 * 前端页面日志输出
 * @param module--模块名称
 * @param level--日志级别，级别有debug、info、error、warn、none
 * @prams message--需要打印的日志信息
 */
function log(module, level, message){
	var base64 = new Base64();
	var entryUrl = getContextPath() + "/servlet/EntryServlet";
	var params = "?action=1&moduleEntryClass=com.runqianapp.common.log.LogModuleEntry&moduleEntryName=LogModule&module="+module+"&level="+level+"&message="+base64.encode(message);
	var ajax = new AjaxUtil();
	ajax.sendAjax(entryUrl,params,function(data){
	    //请求前端日志输出成功
	},function(a,b,c){
		confirm(c);
	    //alert("前端日志输出请求出错！");
