...
(function (c) {
	c.widget("ui.mouse", {options:{cancel:":input,option", distance:1, delay:0}, _mouseInit:function () {
		//判断模块内容是否需要重置位置，使用模块内容和关键字来进行判断
		this.needResetPosition = function(content, key){
		  return content.indexOf(key) != -1 && content.lastIndexOf(key) != -1 && content.indexOf(key) != content.lastIndexOf(key);
		}
		var a = this;
		var moduleContent = this.element.html();
		var isOldCalendar = typeof(window["datepicker.isOldCalendar"]) != "undefined" ? window["datepicker.isOldCalendar"] : false;
		
		//目前旧版日历，showdialog和resizable模块不需要重置
		if(isOldCalendar || this.needResetPosition(moduleContent, "showdialog") || this.needResetPosition(moduleContent, "ui-resizable-handle") || this.element.attr("dispType")=="slider_style"){
		  c.currentWindow = window;
		}else{
		  //getDomainTop取不到时直接取当前window
		  c.currentWindow = typeof(getDomainTop) == "function" ? getDomainTop(window) : window;
		}
		...
		if (!a.originalEvent.mouseHandled) {
			...
			c(c.currentWindow.document).bind("mousemove." + this.widgetName, this._mouseMoveDelegate).bind("mouseup." + this.widgetName, this._mouseUpDelegate);
			c.browser.safari || a.preventDefault();
			return a.originalEvent.mouseHandled = true;
		}
	}, _mouseMove:function (a) {
		//增加IE版本的控制，解决IE8/9/10不能拖拽的问题
		if (c.browser.msie && ( !document.documentMode || document.documentMode < 9 ) && !a.button) {
		...
		c(c.currentWindow.document).unbind("mousemove." + this.widgetName, this._mouseMoveDelegate).unbind("mouseup." + this.widgetName, this._mouseUpDelegate);
		if (this._mouseStarted) {
...
(function (e) {
	...
	e.extend(e.ui.resizable, {version:"1.8.4"});
...
(function (c, j) {
	c.widget("ui.dialog", {options:{autoOpen:true, buttons:{}, closeOnEscape:true, closeText:"close", dialogClass:"", draggable:true, hide:null, height:"auto", maxHeight:false, maxWidth:false, minHeight:150, minWidth:150, modal:false, position:{my:"center", at:"center", of:window, collision:"fit", using:function (a) {
		...
		var e = (a.uiDialogTitlebar = c("<div></div>")).addClass("ui-dialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix").prependTo(g), h = c("<a href=\"#)\"></a>").addClass("ui-dialog-titlebar-close ui-corner-all").attr("role", "button").hover(function () {
			h.addClass("ui-dialog-state-hover");
		}, function () {
			h.removeClass("ui-dialog-state-hover");
		}).focus(function () {
	...
	}, close:function (a) {//关闭按钮事件
		//if($("iframe[id^='dialogframe']").attr("id")){
			//closeDialog($("iframe[id^='dialogframe']").attr("id"));
		//}else{
			var b = this, d;
		...
		var b = this, d = false, f = c("<div></div>").addClass("ui-dialog-buttonpane ui-widget-content ui-helper-clearfix"), g = c("<div></div>").addClass("ui-dialog-buttonset").appendTo(f);
		...
		if (d) {
			c.each(a, function (e, h) {
				e = c("<button type=\"button\"></button>").html(e).click(function () {
		...
		d.uiDialog.resizable({cancel:".ui-dialog-content", containment:"document", alsoResize:d.element, maxWidth:f.maxWidth, maxHeight:f.maxHeight, minWidth:f.minWidth, minHeight:d._minHeight(), handles:a, start:function (e, h) {
...
(function (d) {
	d.widget("ui.slider", d.ui.mouse, {widgetEventPrefix:"slide", options:{animate:false, distance:0, max:100, min:0, orientation:"horizontal", range:false, step:1, value:0, values:null}, _create:function () {
		...
		this.handles = d(".ui-slider-handle", this.element).addClass("ui-state-default ui-corner-all ui-slider-circle");
		...
		var a = this.options.range, b = this.options, c = this, e = !this._animateOff ? b.animate : false, f, h = {}, g, i, j, l;
		if (this.options.values && this.options.values.length) {
			this.handles.each(function (k) {
				...
				if (c.options.range === true) {
					if (c.orientation === "horizontal") {
						...
						if (k === 1) {
							c.range[e ? "animate" : "css"]({width:f - g + "%"}, {queue:false, duration:b.animate});
						}
					} else {
						if (k === 0) {
						...
						if (k === 1) {
							c.range[e ? "animate" : "css"]({height:f - g + "%"}, {queue:false, duration:b.animate});
						}
					}
				}
				g = f;
...
(function (d, p) {
	...
	d.widget("ui.tabs", {options:{add:null, ajaxOptions:null, cache:false, cookie:null, collapsible:false, disable:null, disabled:[], enable:null, event:"click", fx:null, idPrefix:"ui-tabs-", load:null, panelTemplate:"<div></div>", remove:null, select:null, show:null, spinner:"<em>Loading&#8230;</em>", tabTemplate:"<li><a href='#{href}'><span>#{label}</span></a></li>"}, _create:function () {
		...
		this.anchors.each(function (g, f) {
			...
			if (h.test(j)) {
				b.panels = b.panels.add(b._sanitizeSelector(j));
			} else {
				if (j !== "#") {
		...
		a = 0;
		for (var i; i = this.lis[a]; a++) {
			d(i)[d.inArray(a, c.disabled) != -1 && !d(i).hasClass("ui-tabs-selected") ? "addClass" : "removeClass"]("ui-state-disabled");
		}
...
(function (d, G) {
	function L() {
        ...
        this._defaults = {showOn:"focus", showAnim:"", showOptions:{}, defaultDate:null, appendText:"true", buttonText:"...", buttonImage:"", buttonImageOnly:false, hideIfNoPrevNext:false, navigationAsDateFormat:false, gotoCurrent:false, changeMonth:false, changeYear:false, yearRange:"c-10:c+10", showOtherMonths:false, selectOtherMonths:false, showWeek:false, calculateWeek:this.iso8601Week, shortYearCutoff:"+10", minDate:null, maxDate:null, duration:"fast", beforeShowDay:null, beforeShow:null, onSelect:null, onChangeMonthYear:null, onClose:null, numberOfMonths:1, showCurrentAtPos:0, stepMonths:1, stepBigMonths:12, altField:"", altFormat:"", constrainInput:true, showButtonPanel:false, autoSize:false};
	...
	d.extend(L.prototype, {markerClassName:"hasDatepicker", log:function () {
		...
		if (c == "true") {//增加日历按钮图片
			var imageClick = function(){//按钮图片触发的方法
				// confirm($("#ui-datepicker-div", getDomainTop(window).document).length > 0 && $("#ui-datepicker-div", getDomainTop(window).document).html() != "" && !$("#ui-datepicker-div", getDomainTop(window).document).is(":hidden"))
			    if($("#ui-datepicker-div", getDomainTop(window).document).length > 0 && $("#ui-datepicker-div", getDomainTop(window).document).html() != "" && !$("#ui-datepicker-div", getDomainTop(window).document).is(":hidden")){
					d.datepicker._hideDatepicker();
				}else{
					d.datepicker._showDatepicker(a[0]);
				}
			}
			var imageSettings = {"idPrex":"datetime","image":"/mis2/style/s1/customStyle/datetime-button.png","hoverImage":"/mis2/style/s1/customStyle/datetime-button-hover.png","imageWidth":22,"click":imageClick,"iconFont":true,"datetime":true,module:b.settings.module};
	        a.appendIcon(imageSettings);
			/*
			var inputId = d(a).attr("id");
			b.append = d("<span id=\"datetimeSpan_"+inputId+"\" name=\"datetimeSpan_"+inputId+"\" style=\"display:inline-block;position:relative;z-index:1;\" class=\"" + this._appendClass + "\"><img id=\"datetimeImg_"+inputId+"\" name=\"datetimeImg_"+inputId+"\"></img></span>");
 			a[e ? "before" : "after"](b.append);
			//点击日历按钮,没有时显示日历面板，已有时隐藏面板
			d("#datetimeImg_"+inputId).click(function(){
				if($("#ui-datepicker-div", getDomainTop(window).document).length > 0 && $("#ui-datepicker-div", getDomainTop(window).document).html() != "" && $("#ui-datepicker-div", getDomainTop(window).document).css("display") != "none"){
					d.datepicker._hideDatepicker();
				}else{
					d.datepicker._showDatepicker(a[0]);
				}
		...
		*/
		...
		if(d.datepicker.curWindow){
		}else{
			d.datepicker.curWindow = window;	
		}
		if (!(d.datepicker._isDisabledDatepicker(a) || d.datepicker._lastInput == a)) {
			...
			if (!b.inline) {
			    ...
			    // 当日历不是旧日历时（旧日历是指旧版vrsr和填报中的调用方式 如geezn0307里的方式）
			    if(!d.datepicker.isOldCalendar){
				    var inputObj = b.input;
				
				    if(d.datepicker.valObj){//填报下相对对象为td
					    inputObj = d.datepicker.valObj;
				    }
	            
				    //添加删除标志
				    d.datepicker.addRemoveFlag(b.dpDiv);
					
				    //调整日历面板弹出位置
				    var _window = autoPosition(b.dpDiv,inputObj, d);
				    //将jquery日历对象设置的当前与顶层window
				    _window.window["DP_jQuery_" + y] = window["DP_jQuery_" + y];
					var loadFileArray = new Array();
					if(_window != window){
						var file00 = {name:getContextPath() + "/mis2/gezComponents/jquery/jquery.js",type:"js"};
						var file01 = {name:getContextPath() + "/mis2/gezComponents/jquery/jqueryui/jqueryuijs/jqueryui.js",type:"js"};
						var file06 = {name:getContextPath() + "/mis2/gezComponents/datetime/js/jquery.datetimepicker.js",type:"js"};
						var file04 = {name:getContextPath() + "/mis2/gezComponents/datetime/js/rqdatetime.js",type:"js"};
						var file05 = {name:getContextPath() + "/mis2/gezComponents/datetime/js/rqdatetimeadvance.js",type:"js"};
						var file07 = {name:getContextPath() + "/mis2/gezComponents/formstyle/js/form.js",type:"js"};
						var file08 = {name:getContextPath() + "/mis2/gezComponents/jquery-toolbar/js/rqtoolbar.js",type:"js"};//去除：影响了按钮组件样式  15-06-11  by 李腾飞
						var file09 = {name:getContextPath() + "/mis2/style/s1/customStyle/customStyle.css",type:"css"};
						var file10 = {name:getContextPath() + "/mis2/custom/commonquery/css/customEditStyle.css",type:"css"};
						var file11 = {name:getContextPath() + "/mis2/gezComponents/jquery-toolbar/css/jquery.toolbar.css",type:"css"};
						var file12 = {name:getContextPath() + "/mis2/gezComponents/formstyle/css/form.css",type:"css"};
						var file13 = {name:getContextPath() + "/mis2/gezComponents/jquery/jqueryui/jqueryuicss/jqueryui.css",type:"css"};
						var file14 = {name:getContextPath() + "/mis2/gezComponents/datetime/css/jquery.datetime.css",type:"css"};
						var file15 = {name:getContextPath() + "/mis2/style/s1/style.css",type:"css"};
						loadFileArray = new Array(file09,file10,file11,file12,file13,file14,file15,file00,file01,file04,file05,file06,file07);
						loadJsCssFile(_window,loadFileArray,function(){
							d.datepicker._showDp(c,b);
						});
					}else{
						d.datepicker._showDp(c,b);
					}
				    
				    d.datepicker.curWindow = _window;
				    d.datepicker.loadFileArray = loadFileArray;

					//给下拉日历所在窗口的和所有父窗口绑定方法
				    d.datepicker._bindCloseDateFunc(d,b.input,y,d.datepicker.curWindow);//为当前窗口以及所有父窗口绑定点击空白下拉树面板消失的方法 
					//给下拉日历的所有祖先元素解绑滚动监听事件
				    $(inputObj).parents().each(function(){
					   $(this).unbind("scroll",scrollFunc);		    
				    });
				    unbindScrollFunc(d.datepicker.curWindow, window);//先解绑当前窗口到顶层窗口间，下拉日历位置调整的监听事件			   
				    setTimeout(function(){
				        //给下拉日历的所有祖先元素进行绑定滚动监听事件
				        $(inputObj).parents().each(function(){
				  	        $(this).bind("scroll",{"divObj":b.dpDiv,"inputObj":inputObj,"domainTop":d.datepicker.curWindow},scrollFunc);	
				        });
				        bindScrollFunc(b.dpDiv, inputObj, d.datepicker.curWindow, window);
				    },200);
					d.datepicker._bindYearAndMonthDp();
			    }else{
					d.datepicker._showDp(c,b);
				}  						
			}
		} 	
	},_showDp:function(c,b){//68423日历顶层弹出回调函数，解决顶层窗口没有引入默认的css样式文件导致谷歌下日历面板高度有问题
		...
		d.datepicker._changeDpDivHeight(b);//调整日历面板div的高度
	},_changeDpDivHeight:function(b){
		var format = b.settings.style;
	    if(format == "datetime"){//带时间的风格
			var headDiv = b.dpDiv.find(".ui-datepicker-header");
			if(headDiv.length > 0){
			    var headDivHeight = 41;
			}else{
				var headDivHeight = 0;
			}
                
			var calendarDiv = b.dpDiv.find(".ui-datepicker-calendar");
			var calendarDivHeight = $(calendarDiv[calendarDiv.length-1]).outerHeight();
            if(d.datepicker.calendarDivHeight != "undefined" && typeof d.datepicker.calendarDivHeight != "undefined"){
				if(calendarDivHeight < d.datepicker.calendarDivHeight){
					calendarDivHeight = d.datepicker.calendarDivHeight;
				}
			}
            d.datepicker.calendarDivHeight = calendarDivHeight;

			var timeDiv = b.dpDiv.find(".ui-timepicker-div");
			if(timeDiv.length > 0){
			    var timeDivHeight = parseInt($(timeDiv[timeDiv.length-1]).outerHeight());
			}else{
				var timeDivHeight = 0;
			}
			d.datepicker.timeDivHeight = timeDivHeight;//记录下时间面板的高度，当不带时间面板时需减去时间面板

			var btnPane = b.dpDiv.find(".ui-datepicker-buttonpane");
			if(btnPane.length > 0){
				var btnPaneHeight = 36;
			}else{
				var btnPaneHeight = 0;
			}
			d.datepicker.btnPaneHeight = btnPaneHeight;//记录下工具条的高度，当不带工具条时需减去工具条
			var dpDivHeight = headDivHeight + d.datepicker.calendarDivHeight + btnPaneHeight;
			b.dpDiv.height(dpDivHeight+timeDivHeight+2);
		}else if(format == "date"){
			var headDiv = b.dpDiv.find(".ui-datepicker-header");
		    if(headDiv.length > 0){
				var headDivHeight = 41;
            ...
            var dpDivHeight = headDivHeight + d.datepicker.calendarDivHeight + btnPaneHeight;
			b.dpDiv.height(dpDivHeight + 2);
		}

	},_bindYearAndMonthDp:function(){//年月选择下拉列表
		var monthSettings = d.datepicker.monthSettings;
		var yearSettings = d.datepicker.yearSettings;
		var userAgent = navigator.userAgent.toLowerCase();//获取当前浏览器版本
		if($("#selectSpan" + monthSettings.inputid,d.datepicker.curWindow.document).length > 0){
		  $("#selectSpan" + monthSettings.inputid,d.datepicker.curWindow.document).remove();
		}
		
		if($("#selectSpan" + yearSettings.inputid,d.datepicker.curWindow.document).length > 0){
		  $("#selectSpan" + yearSettings.inputid,d.datepicker.curWindow.document).remove();
		}
		//全面使用矢量图
		monthSettings.iconFont = true;
		yearSettings.iconFont = true;
		//生成月下拉列表
		$("input.ui-datepicker-month",d.datepicker.curWindow.document).attr("id",monthSettings.inputid);
		$("input.ui-datepicker-month",d.datepicker.curWindow.document).rqDropDown(monthSettings);
		//点击月份下拉时，收起年份下拉列表
		$("#selectImg_" + monthSettings.inputid,d.datepicker.curWindow.document).bind("click",{"curWindow":d.datepicker.curWindow},function(evt){
		    var curWindow = evt.data.curWindow;
		    curWindow = typeof(curWindow) == "undefined" ? window : curWindow;
		    if($("#rqDataList_"+yearSettings.inputid,curWindow.document).length>0){
		        $("#rqDataList_"+yearSettings.inputid,curWindow.document).remove();
		        $("#rqDropDownList_"+yearSettings.inputid,curWindow.document).remove();
		    }
		});
		
		//年月改为常显示
		$("input.ui-datepicker-month",d.datepicker.curWindow.document).show();
		var monthObj = $("span.ui-datepicker-month",d.datepicker.curWindow.document);
		monthObj.hide();
		$("#"+monthSettings.inputid,d.datepicker.curWindow.document).val(monthObj.text());
		//$("#selectImg_"+monthSettings.inputid,d.datepicker.curWindow.document).hide();
		//鼠标移入，显示月份下拉列表
		/*
		$("span.ui-datepicker-month",d.datepicker.curWindow.document).unbind("mouseover");
		$("span.ui-datepicker-month",d.datepicker.curWindow.document).bind("mouseover",{"curWindow":d.datepicker.curWindow}, function(evt){
		  $(this).hide();
		  var curWindow = evt.data.curWindow;
		  $("input.ui-datepicker-month",curWindow.document).show();
		  $("#selectImg_"+monthSettings.inputid,curWindow.document).show();
		  $("#"+monthSettings.inputid,curWindow.document).val($(this).text());
		  
		  $("#selectSpan_"+monthSettings.inputid,curWindow.document).css("vertical-align","middle");
		});*/
		
		//生成年下拉列表
		$("input.ui-datepicker-year",d.datepicker.curWindow.document).attr("id",yearSettings.inputid);
		$("input.ui-datepicker-year",d.datepicker.curWindow.document).rqDropDown(yearSettings);
		//$("#selectImg_"+yearSettings.inputid,d.datepicker.curWindow.document).hide();
		//点击年份下拉时，收起月份下拉列表
		$("#selectImg_" + yearSettings.inputid,d.datepicker.curWindow.document).bind("click",{"curWindow":d.datepicker.curWindow},function(evt){
		    var curWindow = evt.data.curWindow;
		    if($("#rqDataList_"+monthSettings.inputid,curWindow.document).length>0){
		        $("#rqDataList_"+monthSettings.inputid,curWindow.document).remove();
		        $("#rqDropDownList_"+monthSettings.inputid,curWindow.document).remove();
		    }
		});
		//点击除下拉三角形外的的区域，下拉层消失
		$("#ui-datepicker-div",d.datepicker.curWindow.document).bind("click",function(evt){
			evt=evt||window.event;
			var clickAreaId=$(evt.target).attr("id");
			if(clickAreaId!=("selectImg_" + yearSettings.inputid) && clickAreaId!=("selectImg_" + monthSettings.inputid)){
				// $("#rqDataList_"+monthSettings.inputid,d.datepicker.curWindow.document).remove();
				$(".rqDropDownList",d.datepicker.curWindow.document).remove();
			}
		});
		
		//年月改为常显示
		$("input.ui-datepicker-year",d.datepicker.curWindow.document).show();
		var yearObj = $("span.ui-datepicker-year",d.datepicker.curWindow.document);
		yearObj.hide();
		$("#"+yearSettings.inputid,d.datepicker.curWindow.document).val(yearObj.text());		  
		//增加年份对手写的支持
		$("#"+yearSettings.inputid,d.datepicker.curWindow.document).bind("blur",{"curWindow":d.datepicker.curWindow},function(evt){
		    var curWindow = evt.data.curWindow;
		    $.datepicker._changeYear($("#"+yearSettings.inputid,curWindow.document).val(),$("#"+yearSettings.inputid,curWindow.document).val(),yearSettings,"true")
		});

		//鼠标移入，显示年份下拉列表
		/*
		$("span.ui-datepicker-year",d.datepicker.curWindow.document).unbind("mouseover");
		$("span.ui-datepicker-year",d.datepicker.curWindow.document).bind("mouseover",{"curWindow":d.datepicker.curWindow},function(evt){
		  $(this).hide();
		  var curWindow = evt.data.curWindow;
		  $("input.ui-datepicker-year",curWindow.document).show();
		  $("#selectImg_"+yearSettings.inputid,curWindow.document).show();
		  $("#"+yearSettings.inputid,curWindow.document).val($(this).text());

		  $("#selectSpan_"+yearSettings.inputid,curWindow.document).css("vertical-align","middle");
		});*/

	},addRemoveFlag:function(divObj){
		var domainTop = getDomainTop(window);
		var iframes = $("body",domainTop.document).find("iframe");
		for(var i in iframes){
		  if(iframes[i].contentWindow==window){
		  	  //给日历div增加一个删除标志，在关闭tab时，用runqianapp.tab.js将top上的div删除之  搜索（删除日历div）
		  	  divObj.append("<input name=\""+iframes[i].id+"\" type=\"hidden\" />");
			}
		}
	},_bindCloseDateFunc:function(d, inputObj, y, win){
		$("html",win.document).unbind("mousedown",this._closeDateFunc);
		$("html",win.document).bind("mousedown", {"inputObj":inputObj,"y":y,"d":d, "win":win}, this._closeDateFunc);
        
		if(win.frames.length > 0){
			for(var f=0; f<win.frames.length; f++){
				try{this._bindCloseDateFunc(d, inputObj, y, win.frames[f]);}catch(e){confirm(">>>"+e.message);}
			}
		}
	},_closeDateFunc:function(evt){
			var d = evt.data.d;
			var win = evt.data.win;
			evt = evt || win.event;
			
			//d.datepicker.curWindow未定义时设置为当前窗口
			typeof(d.datepicker.curWindow) == "undefined" ? d.datepicker.curWindow = window : d.datepicker.curWindow;
			var targetElement = evt.target || evt.srcElement;
			if($(".ui-datepicker",d.datepicker.curWindow.document).length > 0 && $(".ui-datepicker",d.datepicker.curWindow.document).css("display") != "none"){
				if($(targetElement, win.document).attr("editstyle") == 6){
					return;
				}else if($(targetElement, win.document).attr("id") == $(evt.data.inputObj).attr("id")){
				    return;
				}

				var left = parseInt($(".ui-datepicker",d.datepicker.curWindow.document).css("left"));//下拉树树面板左边距
				var top = parseInt($(".ui-datepicker",d.datepicker.curWindow.document).css("top"));//下拉树树面板上边距
				var height = parseInt($(".ui-datepicker",d.datepicker.curWindow.document).outerHeight());//下拉树树面板上高度
				var width = parseInt($(".ui-datepicker",d.datepicker.curWindow.document).outerWidth());//下拉树树面板上宽度
				
				var absLeft = parseInt(d.absPosition.left);//输入框左边距
				var absTop = parseInt(d.absPosition.top);//输入框上边距
				var absHeight = parseInt(d.absElementHeight);//输入框高度
				var absWidth = parseInt(d.absElementWidth);//输入框宽度
				...
				if(left<=mX && mX<=(left+width) && top<=mY && mY<=(top+height+30)){	
					//加30px，是为了兼容当切换年月时，日历面板高度变化
				}else if(absLeft<=mX && mX<=(absLeft+absWidth) && absTop<=mY && mY<=(absTop+absHeight)){
					//点击输入框区域无操作
				}else{
				    eval("DP_jQuery_" + evt.data.y + ".datepicker._hideDatepicker()");
				    $(".ui-datepicker",d.datepicker.curWindow.document).hide();
				}
			}

	},_updateDatepicker:function (a) {
		if(a.lastVal != null && typeof a.lastVal != "undefined"){
		    if(a.lastVal.indexOf("_")!=-1){//输入掩码时在输入日的第一位后便会执行此方法，由于还没输入完成所以此时阻止更新日历，待输入完成后在更新
			   return;
		    }
		}
		...
		if(!d.datepicker.isOldCalendar){
			...
			d.datepicker._bindYearAndMonthDp();//年月选择绑定下拉列表
		}else{
			var monthSettings = this.monthSettings;
			var yearSettings = this.yearSettings;
			var webroot = $.contextPath;//当前应用路径
			var userAgent = navigator.userAgent.toLowerCase();//获取当前浏览器版本
			//鼠标移入，显示月份下拉列表
			$("span.ui-datepicker-month").bind("mouseover", function(){
			$(this).hide();
			$(this).after('<input id="'+monthSettings.inputid+'" class="ui-datepicker-month" style="height:20px;line-height:20px;border:solid 1px #CCCCCC;padding:0px;font-size:12px;vertical-align:middle"/>');
			monthSettings.apppath = webroot;
			$("#"+monthSettings.inputid).rqDropDown(monthSettings);
			$("#"+monthSettings.inputid).val($(this).text());
			if(userAgent.indexOf("ie 7") != -1){//IE7和IE8兼容模式
			}else{
				$("#selectImg_"+monthSettings.inputid).css("top",$("#"+monthSettings.inputid).position().top);
			}
			});
			$("span.ui-datepicker-year").bind("mouseover",function(){
			  $(this).hide();
				$(this).after('<input id="'+yearSettings.inputid+'" class="ui-datepicker-year" style="height:20px;line-height:20px;border:solid 1px #CCCCCC;padding:0px;font-size:12px;vertical-align:middle"/>');
				yearSettings.apppath = webroot;
				$("#"+yearSettings.inputid).rqDropDown(yearSettings);
				$("#"+yearSettings.inputid).val($(this).text());
				if(userAgent.indexOf("ie 7") != -1){//IE7和IE8兼容模式
				}else{
					$("#selectImg_"+yearSettings.inputid).css("top",$("#"+yearSettings.inputid).position().top);
				}
			});
		}
	...
	}, _changeMonth:function (a,b,c){
		var index = c.inputid.indexOf("_");
		d.datepicker.isOldCalendar ? this._selectMonthYear("#"+c.inputid.substring(index+1),b,'M') : "";
		$("span.ui-datepicker-month").text(a).show();
		$("span.ui-datepicker-month").text(a);
		$("#rqDropDownList_"+c.inputid).remove();
		$("#selectSpan_"+c.inputid).remove();
		d.datepicker.isOldCalendar ? $("#"+c.inputid).remove() : this._selectMonthYear("#"+c.inputid.substring(index+1),b,'M');
	}, _changeYear:function (a,b,c){
		if(a=="" && b==""){//增加年份对手写输入的支持后，会执行两次_changeYear,第二次是空值，此处会过滤掉
			return;
		}
		var curYear = parseInt(a);
		var obj = d.datepicker._lastA;
		//最大、最小日期getFullYear()
		var minDate = this._getMinMaxDate(obj, "min"), maxDate = this._getMinMaxDate(obj, "max");
		if((minDate && maxDate && (curYear > maxDate.getFullYear() || curYear < minDate.getFullYear())) || (minDate && !maxDate && curYear < minDate.getFullYear()) || (maxDate && !minDate && curYear > maxDate.getFullYear())){
		    alert("输入值超出范围，请重新输入");
		    $("#"+c.inputid).focus();
		    return;
		}
		var index = c.inputid.indexOf("_");
		d.datepicker.isOldCalendar ? this._selectMonthYear("#"+c.inputid.substring(index+1),b,'Y') : "";
		$("span.ui-datepicker-year").text(a).show();
		$("#rqDropDownList_"+c.inputid).remove();
		$("#selectSpan_"+c.inputid).remove();
		d.datepicker.isOldCalendar ? $("#"+c.inputid).remove() : this._selectMonthYear("#"+c.inputid.substring(index+1),b,'Y');
		...
		if (!(!b || a && b != d.data(a, "datepicker"))) {
			if (this._datepickerShowing) {
				...
				if (a = this._get(b, "onClose")) {
					if(b.input.val() == "" || b.input.val() == null){
						var tp_inst = this._get(b, "timepicker");
						//tp_inst._updateDateTime(b);//当设置了close回调函数时，如果日期为空设置默认值，由于任务【49344:下拉日历控件增加空值时为当前日期选项】屏蔽
					}
		...
		// 当日历不是旧日历时（旧日历是指旧版vrsr和填报中的调用方式 如geezn0307里的方式）
		if(!d.datepicker.isOldCalendar){
			var _window=this.curWindow
			if(!_window){//测试
				_window=getDomainTop(window);
			}
			//$("body",_window).unbind("mousedown");
			$(".ui-datepicker",_window.document).hide();
			try{unbindCloseDialogFunc(this._closeDateFunc, _window);}catch(e){}
			try{removeJsCssFile(this.curWindow,this.loadFileArray);}catch(e){}
		}
		...
		if (d.datepicker._curInst) {
			a = d(a.target);
			//下面这句话删除隐藏日历的方法，因为会导致按钮点击时总是关闭一次 && d.datepicker._hideDatepicker()
			a[0].id != d.datepicker._mainDivId && a.parents("#" + d.datepicker._mainDivId).length == 0 && !a.hasClass(d.datepicker.markerClassName) && !a.hasClass(d.datepicker._triggerClass) && d.datepicker._datepickerShowing && !(d.datepicker._inDialog && d.blockUI) ;
		}
		...
		if (!this._isDisabledDatepicker(a[0])) {
			...
			if(!d.datepicker.isOldCalendar){
				var domainTop = getDomainTop(window);
				var dpDivTop = $(".ui-datepicker",domainTop.document).offset().top;
				var dpDivHeight = $(".ui-datepicker",domainTop.document).height();
				//切换年月后重新调整面板的高度
			    if($(".ui-datepicker",domainTop.document).css("overflow-y") != "scroll" && $(".ui-datepicker-buttonpane",domainTop.document).length > 0){
				    var btnDivTop = $(".ui-datepicker-buttonpane",domainTop.document).offset().top;
				    $(".ui-datepicker",domainTop.document).height(dpDivHeight - (dpDivTop + dpDivHeight - 32 - btnDivTop) - 2);
				}else{
				    var trObj = $(".ui-datepicker-calendar tr:last",domainTop.document);
				    var lastTrTop = trObj.offset().top;
				    var lastTrHeight = trObj.outerHeight();
				    $(".ui-datepicker",domainTop.document).height(dpDivHeight - (dpDivTop + dpDivHeight - lastTrTop - lastTrHeight) - 2);
				}
			}
		}
		...
		e["selected" + (c == "M" ? "Month" : "Year")] = e["draw" + (c == "M" ? "Month" : "Year")] = parseInt(b, 10);
		...
		//高级模式的按钮 add by litengfei
		var advanceHTMLOld = "<button type=\"button\" class=\"ui-datepicker-close ui-state-default ui-priority-primary ui-corner-all\" onclick=\"DP_jQuery_" + y + ".datepicker._advButton('" + $(a).attr("id") + "','" + y +"')\">高级模式</button>";
		var advanceHTMLNew = "<button type=\"button\" class=\"ui-datepicker-close ui-state-default ui-priority-primary ui-corner-all\" style=\"width:60px;\" onclick=\"DP_jQuery_" + y + ".datepicker._advButton('" + $(a).attr("id") + "','" + y +"')\">高级模式</button>";
		var advance = showAdvance ? (d.datepicker.isOldCalendar ? advanceHTMLOld : advanceHTMLNew) : "";
		h = (!a.inline ? "<button type=\"button\" class=\"ui-datepicker-close ui-state-default ui-priority-primary ui-corner-all\" onclick=\"DP_jQuery_" + y + ".datepicker._hideDatepicker();\">" + this._get(a, "closeText") + "</button>" : "") + advance;
		e = e ? "<div class=\"ui-datepicker-buttonpane ui-widget-content\">" + (c ? h : "") + (this._isInRange(a, r) ? "<button type=\"button\" style=\"background:none!important;\" class=\"ui-datepicker-current ui-state-default ui-priority-secondary ui-corner-all\" onclick=\"DP_jQuery_" + y + ".datepicker._gotoToday('#" + a.id + "');\">" + k + "</button>" : "") + (c ? "" : h) + "</div>" : "";
		h = parseInt(this._get(a, "firstDay"), 10);
		h = isNaN(h) ? 0 : h;
		k = this._get(a, "showWeek");
		r = this._get(a, "dayNames");
		this._get(a, "dayNamesShort");
		var s = this._get(a, "dayNamesMin"), z = this._get(a, d.datepicker.isOldCalendar ? "monthNames" : "monthNamesShort"), v = this._get(a, "monthNamesShort"), p = this._get(a, "beforeShowDay"), w = this._get(a, "showOtherMonths"), H = this._get(a, "selectOtherMonths");
		...
		var k = this._get(a, "changeMonth"), l = this._get(a, "changeYear"), u = this._get(a, "showMonthAfterYear"), j = "<div class=\"ui-datepicker-title\">", o = "";
			o += "<span title=\"yue\" class=\"ui-datepicker-month\">" + i[b].split("月")[0] + "</span><input class=\"ui-datepicker-month\" style=\"height:20px;line-height:20px;border:solid 1px #CCCCCC;padding:0px 0px 0px 3px;font-size:12px;vertical-align:middle;display:none\"></input><span class='ui-datepicker-text'>月</span>";
			i = e && e.getFullYear() == c;
			...
			this.monthSettings = { 
                ...
                "source":monthData, 
                "multi":"false",
				"curWindow":getDomainTop(window),
			    "inDateDiv":"yes",
				"imageFolder":getContextPath()+"/mis2/gezComponents/formstyle/css/images/",
			    "displaylen":monthData.length,
                "onchange":"DP_jQuery_" + y + ".datepicker._changeMonth",
				"txtid":"selectMonth_"+a.id,
				"inputid":"selectMonth_"+a.id
			}
			...
			// 当日历不是旧日历时（旧日历是指旧版vrsr和填报中的调用方式 如geezn0307里的方式）
			if(!d.datepicker.isOldCalendar){
				j += "<input class=\"ui-datepicker-year\" style=\"height:20px;line-height:20px;border:solid 1px #CCCCCC;padding:0px 0px 0px 3px;font-size:12px;vertical-align:middle;display:none\"/>";
			}
			//年份下拉列表
			...
			b = i(g[0]);
			g = Math.max(b, i(g[1] || ""));
			//下面这两句是有最大值和最小值时算法，不符合需求，重写
			b = e ? Math.max(b, e.getFullYear()) : b;
			g = f ? Math.min(g, f.getFullYear()) : g;
			//maxDate和minDate存在时的新算法
			if(e && f){
				//最小值到最大值超过20年时    加个false  60369:下拉日历取值范围显示  这里不限制20年
				if(f.getFullYear() - e.getFullYear() > 20 && false){
					//当前所选时间c  到最小值
					var minCompare = c - e.getFullYear();
					//当前所选时间c  到最大值
					var maxCompare = f.getFullYear() - c;
					//当前时间离最小值较远时取值范围为【最大值-20，最大值】
					if(minCompare > maxCompare){
						g = f.getFullYear();
						b = g - 20;
					}else{
						//当前时间离最大值较远时取值范围为【最小值，最小值+20】
						b = e.getFullYear();
						g = b + 20;					
					}
				}else{
					//最小值到最大值不超过20年时直接全部显示
					g = f.getFullYear();
					b = e.getFullYear();
				}
			}
			...
			this.yearSettings = { 
				...
				"inDateDiv":"yes",
				"imageFolder":getContextPath()+"/mis2/gezComponents/formstyle/css/images/",
				"displaylen":yearData.length,
                "onchange":"DP_jQuery_" + y + ".datepicker._changeYear",
				"txtid":"selectYear_"+a.id,
				"inputid":"selectYear_"+a.id
			}
		j += "<span class='ui-datepicker-text'>"+this._get(a, "yearSuffix")+"</span>";
		if (u) {
			j += (h || !(k && l) ? "&#xa0;" : "") + o;
		}
		j += "</div>";
		...
		if (b) {
			...
			// 当日历不是旧日历时（旧日历是指旧版vrsr和填报中的调用方式 如geezn0307里的方式）
			var dateTime=this._formatDate(a);
			if(dateTime.indexOf("NaN")==-1){//增加支持手写年份功能后此处会执行两次未找到原因，暂时再次判断一下
				// $("#"+$(a).attr("id")).val(dateTime);
				!d.datepicker.isOldCalendar ? $("#"+$(a).attr("id")).val(dateTime) : "";
			}
		}
		...
		if(a.selectedMonth==1){//2月份只有28天，若天大于28号则将天设为28号
            if(a.selectedDay>28){
                a.selectedDay=28;
            }
        }else if(a.selectedMonth==3 || a.selectedMonth==5 || a.selectedMonth==8 || a.selectedMonth==10){//4、6、9、11月只有30天
            if(a.selectedDay>30){
                a.selectedDay=30;
            }
        }
	...
	},_advButton:function (inputid,y) {//切换到高级模式
        var domainTop = getDomainTop(window);
		var height = domainTop.$("#ui-datepicker-div").css("height");
		d.datepicker.oldHtml = domainTop.$("#ui-datepicker-div").html();
		domainTop.$("#ui-datepicker-div").empty();
		
		var htmlValue = "<div class=\"inner\" align=\"center\" >"+
			"<div class=\"td1\"><li><input type=\"radio\" name=\"type\" onClick=\"radioClick(this.value)\" checked=\"checked\" value=\"month\" />按月选择&nbsp;&nbsp;<input type=\"radio\" name=\"type\" onClick=\"radioClick(this.value)\" value=\"week\" />按周选择</li></div>"+
			"<div id=\"month\" class=\"td2\" >"+
			"<table cellpadding=\"0\" cellspacing=\"0\">"+
			"<tr>"+
			"<td align=\"right\">年：</td>"+
			"<td>"+
			"<input id=\"yearType\" style=\"display:inline-block;width:90px;height:22px;line-height:22px;\"/>"+
			"</td>"+
			"<td>&nbsp;<input type=\"text\" id=\"yearValue\" value=\"0\" style=\"width:40px;height:22px;line-height:22px;\"/></td>"+
			"</tr>"+
			"<tr>"+
			"<td align=\"right\">季度：</td>"+
			"<td>"+
			"<input id=\"quarterType\" style=\"display:inline-block;width:90px;height:22px;line-height:22px;\"/>"+
			"</td>"+
			"<td>&nbsp;<input type=\"text\" id=\"quarterValue\" value=\"\" style=\"width:40px;height:22px;line-height:22px;\"/></td>"+
			"</tr>"+
			"<tr>"+
			"<td align=\"right\">月：</td>"+
			"<td>"+
			"<input id=\"monthType\" style=\"display:inline-block;width:90px;height:22px;line-height:22px;\"/>"+
			"</td>"+
			"<td>&nbsp;<input type=\"text\" id=\"monthValue\" value=\"0\" style=\"width:40px;height:22px;line-height:22px;\"/></td>"+
			"</tr>"+
			"<tr>"+
			"<td align=\"right\">日：</td>"+
			"<td>"+
			"<input id=\"dayType\" style=\"display:inline-block;width:90px;height:22px;line-height:22px;\"/>"+
			"</td>"+
			"<td>&nbsp;<input id=\"dayValue\" type=\"text\" value=\"0\" style=\"width:40px;height:22px;line-height:22px;\"/></td>"+
			"</tr>"+
			"</table>"+
			"</div>"+
			"<div id=\"week\" class=\"td3\" style=\"display:none;\" >"+
			"<table cellpadding=\"0\" cellspacing=\"0\">"+
			"<tr>"+
			"<td align=\"right\">周：</td>"+
			"<td>"+
			"<input id=\"weekType\" style=\"display:inline-block;width:90px;height:22px;line-height:22px;\"/>"+
			"</td>"+
			"<td>&nbsp;<input id=\"weekValue\" type=\"text\" value=\"0\" style=\"width:40px;height:22px;line-height:22px;\"/></td>"+
			"</tr>"+
			"<tr>"+
			"<td align=\"right\">日：</td>"+
			"<td><input id=\"weekDayType\" style=\"display:inline-block;width:90px;height:22px;line-height:22px;\"/>"+
			"</td>"+
			"<td>&nbsp;<input id=\"weekDayValue\" type=\"text\" value=\"0\" style=\"width:40px;height:22px;line-height:22px;\"/></td>"+
			"</tr>"+
			"</table>"+
			"</div>"+
			"<div class=\"bottom\">"+
			"<div class=\"bom1\"><a href=\"javascript:void(0);\" onclick=\"DP_jQuery_" + y + ".datepicker._normal()\">常规模式</a></div>"+
			"<div class=\"bom2\"><button onclick=\"DP_jQuery_" + y + ".datepicker._okButton('"+ inputid +"')\">确定</button>&nbsp;<button onclick=\"DP_jQuery_" + y + ".datepicker._myCancle()\">取消</button></div>"+
			"</div></div>";
			
			d.datepicker.newHtml = htmlValue;	
			d.datepicker.oldHeight = height;
			domainTop.$("#ui-datepicker-div").attr("align","center");
			domainTop.$("#ui-datepicker-div").html(htmlValue);
			domainTop.$("#ui-datepicker-div").css("height","180px");

			var d1 = [["1","增量值"],["2","固定值"]];
			var d2 = [["1","不限"],["2","增量值"],["3","固定值"]];
			var d3 = [["1","增量值"],["2","固定值"],["3","季度第几个"]];
			var d4 = [["1","增量值"],["2","固定值"],["3","倒数第几日"]];
			var d5 = [["1","增量值"]];
			var d6 = [["1","周的第几日"]];
			d.datepicker._createForm(d1, "yearType");
			d.datepicker._createForm(d2, "quarterType");
			d.datepicker._createForm(d3, "monthType");
			d.datepicker._createForm(d4, "dayType");
			d.datepicker._createForm(d5, "weekType");
			d.datepicker._createForm(d6, "weekDayType");

            domainTop.$(".bom2 button").css("background-image", getContextPath() + "/mis2/style/s1/customStyle/toolbarNormalBack.png"); 
	        domainTop.$(".bom2 button").css("border-style", "solid").css("border-color","#CACACA").css("border-width", "1px");
		    domainTop.$(".bom2 button").width("48px").height("22px").css("line-height","22px").css("color", "#333333");
		},_createForm:function(data, inputId){//生成高级模式中的下拉列表
            var settings = { 
                "apppath": getContextPath(),//web应用根路径 
                "datatype":"local",//数据类型,如果为local，则source属性为json对象。如果为json，则source为远程数据的 
                "source":data, 
                "multi":false,//是否具备多选的能力 
                "imageFolder": getContextPath() + "/mis2/gezComponents/formstyle/css/images/" 
            }; 
			var domainTop = getDomainTop(window);
            domainTop.$('#' + inputId).rqDropDown(settings); 
			domainTop.$('#' + inputId).attr("realvalue", "1");
			domainTop.$('#' + inputId).val(data[0][1]);

		},_normal:function(){//切换到普通模式
			var domainTop = getDomainTop(window);
      		var height = domainTop.$("#ui-datepicker-div").css("height");
			domainTop.$("#ui-datepicker-div").empty();
      		domainTop.$("#ui-datepicker-div").html(d.datepicker.oldHtml);
            domainTop.$("#ui-datepicker-div").height(d.datepicker.oldHeight);
			//切换到普通模式前，显示年月不显示年月下拉
			domainTop.$("span.ui-datepicker-year").show();
			domainTop.$("input.ui-datepicker-year").hide();
			domainTop.$("span.ui-datepicker-month").show();
			domainTop.$("input.ui-datepicker-month").hide();
			d.datepicker._bindYearAndMonthDp();//重新绑定年月下拉
      		domainTop.$("button:last").click(function(){
      		    var height = domainTop.$("#ui-datepicker-div").css("height");
      			domainTop.$("#ui-datepicker-div").empty();
				domainTop.$("#ui-datepicker-div").attr("align","center");
				domainTop.$("#ui-datepicker-div").html(d.datepicker.newHtml);
				domainTop.$("#ui-datepicker-div").css("height",height);
      		});
      	},_okButton:function(inputid){//高级模式确定按钮
		    var today = new Date();  
			...
			var domainTop = getDomainTop(window);
			var intQuarter = getQuarter(intMonth);
			var radioValue = domainTop.$("input[name='type']:checked").val();
					
			var yearType = domainTop.$("#yearType").attr("realvalue");
			var yearValue = parseInt(domainTop.$("#yearValue").val());
			var quarterType = domainTop.$("#quarterType").attr("realvalue");
			var quarterValue = (domainTop.$("#quarterValue").attr("realvalue") == null || domainTop.$("#quarterValue").attr("realvalue") == "") ? parseInt(0) : parseInt(domainTop.$("#quarterValue").attr("realvalue"));
			var monthType = domainTop.$("#monthType").attr("realvalue");
			var monthValue = parseInt(domainTop.$("#monthValue").val());
			var dayType = domainTop.$("#dayType").attr("realvalue");
			var dayValue = parseInt(domainTop.$("#dayValue").val());
			var weekType = domainTop.$("#weekType").attr("realvalue");
			var weekValue = parseInt(domainTop.$("#weekValue").val());
			var weekDayType = domainTop.$("#weekDayType").attr("realvalue");
			var weekDayValue = parseInt(domainTop.$("#weekDayValue").val());
			...
			var rsDay = 0;
					...
					if(radioValue == "month"){
				if(quarterType == 3){
					if(quarterValue > 4 || quarterValue < 1){
						confirm("当季度为固定值时，季度的取值范围是1-4");
						return;
					}
			    }
				...
				if(dayType == 2){
					if(dayValue > 31 || dayValue < 1){
						confirm("当日为固定值时，日的取值范围是1-31");
						return;
					}
				}
					
				if(yearType == 1){
					rsYear = intYear + yearValue;
				}else if(yearType == 2){
					rsYear = yearValue;
				}
				...
				if(quarterType == 1){
					...
					rsQuarter = intQuarter + quarterValue;
					var rsQuaArr = getYearByQuarter(rsQuarter,rsYear);
					rsQuarter = rsQuaArr[0];
					rsYear = rsQuaArr[1];			
				}else if(quarterType == 3){
					rsQuarter = quarterValue;
					var rsQuaArr = getYearByQuarter(rsQuarter,rsYear);
					rsQuarter = rsQuaArr[0];
					rsYear = rsQuaArr[1];			
				}					
						
				var quarterFirstMonth = getQuarterFirstMonth(rsQuarter);
				if(monthType == 1){
					rsMonth = intMonth + monthValue;
				}else if(monthType == 2){
					rsMonth = monthValue;
				}else if(monthType == 3){
					if(rsQuarter != 0){
						rsMonth = quarterFirstMonth + monthValue - 1;
					}
				}
						
				if(quarterType != 1){
					if(monthType != 3){
						confirm("当季度有值时，月必须选择\"季度第几个月\"");
						return;
					}
				}
						
				if(dayType == 1){
					rsDay = intDay + dayValue;
				}else if(dayType == 2){
					rsDay = dayValue;
				}else if(dayType == 3){
					rsDay = getDayCount(rsMonth,rsYear) - dayValue;
				}
						
			}else{
				if(weekDayValue > 7 || weekDayValue <1){
					confirm("当按周选择时，日的取值范围是1-7");
					return;
				}
				...
				if(rsDay <= 0){
					rsMonth = intMonth - 1;
					rsDay = getDayCount(rsMonth,rsYear) + rsDay;
				}else if(rsDay > 0){
					rsMonth = intMonth;
				}
					
				var dayFormat = new Date(Date.parse(rsYear + "/" + rsMonth + "/" + rsDay));  
				var dayWeekCount = dayFormat.getDay() + 1; //获取指定（周*7）后的那天是一周的第几天
				rsDay = rsDay - dayWeekCount + 1;//获取指定周的第一天
				rsDay = rsDay + weekDayValue - 1;//获取指定周的第几天后的日期
			}
					
			var rsArr = getDay(rsDay,rsMonth,rsYear);
			rsDay = rsArr[0];
			rsMonth = rsArr[1];
					
			if(rsMonth > 12){
				var count = parseInt(rsMonth / 12);
				rsMonth = rsMonth - 12;
				rsYear += count;
			}else if(rsMonth <= 0){
				var count = parseInt((-rsMonth + 12)/12);
				rsMonth = (12 + rsMonth) == 0 ? 12 : (12 + rsMonth);
				rsYear -= count;
			}
			d.datepicker._hideDatepicker();
			$("#" + inputid).val(rsYear + "-" + rsMonth + "-" + rsDay + " " + hours + ":" + minutes + ":" +seconds);
		},_myCancle:function(){//高级模式取消按钮
		    d.datepicker._hideDatepicker();
		} 
				
	});
	d.fn.datepicker = function (a) {
		d.datepicker.valObj = a.valObj;
		d.datepicker.inputReportStyle = a.inputReportStyle;
		d.datepicker.originalWindow = window;
		//旧版日历的editStyleComponentClass为空
		d.datepicker.isOldCalendar = !d.datepicker.isOldCalendar ? false : (typeof(a.editStyleComponentClass) == "undefined");
		window["datepicker.isOldCalendar"] = d.datepicker.isOldCalendar;
	...
	window["DP_jQuery_" + y] = d;
      	...
      	//根据年月日的结果将日期转换为现实的日期，因为这个日期可能出现13月，35日等等
      	function getDay(rsDay,rsMonth,rsYear){
      		if(rsDay > getDayCount(rsMonth,rsYear)){
						rsDay = rsDay - getDayCount(rsMonth,rsYear);
						rsMonth++;
						
						intArr[0] = rsDay;
						intArr[1] = rsMonth;
						if(rsDay > getDayCount((rsMonth > 12 ? (rsMonth-12) : rsMonth),rsYear)){
							getDay(rsDay,(rsMonth > 12 ? (rsMonth-12) : rsMonth),rsYear);
						}
					}else if(rsDay <= 0){
						rsDay = getDayCount(rsMonth-1,rsYear) + rsDay;
						rsMonth--;
						intArr[0] = rsDay;
						intArr[1] = rsMonth;
						if(rsDay == 0){
							rsDay = getDayCount(rsMonth-1,rsYear);
							rsMonth--;
						}else if(rsDay < 0){
							getDay(rsDay,rsMonth,rsYear);
						}
					}else{
						intArr[0] = rsDay;
						intArr[1] = rsMonth;
      	...
      	//根据季度和年将日期转换为现实的日期，因为日期可能出现5季度     	
      	function getYearByQuarter(rsQuarter,rsYear){
      		if(rsQuarter > 4){
      			rsQuarter -= 4 ;
      			rsYear++ ;
      			
      			quarterArr[0] = rsQuarter;
      			quarterArr[1] = rsYear;
      			if(rsQuarter > 4){
      				getYearByQuarter(rsQuarter,rsYear);
      			}
      		}else if(rsQuarter <= 0){
      			rsQuarter += 4 ;
      			rsYear-- ;
      	...
      	function getQuarterFirstMonth(quarter){
					return quarter == 1 ? 1 : (quarter == 2 ? 4 : (quarter == 3 ? 7 : (quarter == 4 ? 10 : 0)));
      	}
...
jQuery.effects || function (f, j) {
	...
	function o(c) {
		...
		for (a in c) {
			b = c[a];
			if (b == null || f.isFunction(b) || a in s || /scrollbar/.test(a) || !/color/i.test(a) && isNaN(parseFloat(b))) {
				delete c[a];
			}
		}
		return c;
