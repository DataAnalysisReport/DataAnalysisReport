...
(function (d, b) {
	...
	d.ui = d.ui || {};
	if (!d.ui.version) {
		d.extend(d.ui, {version:"1.8.4", plugin:{add:function (f, c, h) {
			f = d.ui[f].prototype;
			for (var g in h) {
			...
			return document.compareDocumentPosition ? e.compareDocumentPosition(c) & 16 : e !== c && e.contains(c);
		}, hasScroll:function (e, c) {
			if (d(e).css("overflow") === "hidden") {
				return false;
			}
		...
		d.fn.extend({_focus:d.fn.focus, focus:function (e, c) {
			...
			if (this.length) {
				e = d(this[0]);
				for (var c; e.length && e[0] !== document; ) {
					c = e.css("position");
					if (c === "absolute" || c === "relative" || c === "fixed") {
						c = parseInt(e.css("zIndex"));
						if (!isNaN(c) && c != 0) {
							return c;
						}
					}
		...
		d.each(["Width", "Height"], function (f, c) {
			function l(n, i, h, e) {
				d.each(k, function () {
					i -= parseFloat(d.curCSS(n, "padding" + this, true)) || 0;
					if (h) {
						i -= parseFloat(d.curCSS(n, "border" + this + "Width", true)) || 0;
					}
					if (e) {
						i -= parseFloat(d.curCSS(n, "margin" + this, true)) || 0;
					}
				});
				return i;
			}
			...
			d.fn["inner" + c] = function (e) {
				if (e === b) {
					return g["inner" + c].call(this);
				}
				return this.each(function () {
					d.style(this, j, l(this, e) + "px");
				});
			};
			d.fn["outer" + c] = function (h, e) {
				if (typeof h !== "number") {
					return g["outer" + c].call(this, h);
				}
				return this.each(function () {
					d.style(this, j, l(this, h, true, e) + "px");
				});
			};
		});
		d.extend(d.expr[":"], {data:function (e, c, f) {
			return !!d.data(e, f[3]);
		}, focusable:function (e) {
			var c = e.nodeName.toLowerCase(), f = d.attr(e, "tabindex");
			if ("area" === c) {
				c = e.parentNode;
				f = c.name;
				if (!e.href || !f || c.nodeName.toLowerCase() !== "map") {
					return false;
				}
				e = d("img[usemap=#" + f + "]")[0];
				return !!e && a(e);
	...
	a.widget.bridge = function (b, e) {
		a.fn[b] = function (k) {
			...
			j ? this.each(function () {
				var h = a.data(this, b), f = h && a.isFunction(h[k]) ? h[k].apply(h, i) : h;
				if (f !== h && f !== d) {
					g = f;
					return false;
				}
				...
				if (f) {
					...
					a.data(this, b, new e(k, this));
				}
			});
			return g;
		};
	};
	a.Widget = function (b, e) {
		arguments.length && this._createWidget(b, e);
	};
	a.Widget.prototype = {widgetName:"widget", widgetEventPrefix:"", options:{disabled:false}, _createWidget:function (b, f) {
...
(function (f) {
	...
	f.fn.position = function (j) {
		...
		k.top += n[1];
		return this.each(function () {
			var p = f(this), h = p.outerWidth(), g = p.outerHeight(), i = f.extend({}, k);
			if (j.my[0] === "right") {
				i.left -= h;
			} else {
				if (j.my[0] === "center") {
			...
			f.each(["left", "top"], function (s, q) {
				f.ui.position[o[s]] && f.ui.position[o[s]][q](i, {targetWidth:m, targetHeight:l, elemWidth:h, elemHeight:g, offset:n, my:j.my, at:j.at});
			});
			f.fn.bgiframe && p.bgiframe();
			p.offset(f.extend(i, {using:j.using}));
		});
	};
	f.ui.position = {fit:{left:function (g, c) {
		var h = f(window);
...
(function (a) {
	a.widget("ui.draggable", a.ui.mouse, {widgetEventPrefix:"drag", options:{addClasses:true, appendTo:"parent", axis:false, connectToSortable:false, containment:false, cursor:"auto", cursorAt:false, grid:false, handle:false, helper:"original", iframeFix:false, opacity:false, refreshPositions:false, revert:false, revertDuration:500, scope:"default", scroll:true, scrollSensitivity:20, scrollSpeed:20, snap:false, snapMode:"both", snapTolerance:20, stack:false, zIndex:false}, _create:function () {
		...
		var d = this.options, l = this.cssPosition == "absolute" && !(this.scrollParent[0] != document && a.ui.contains(this.scrollParent[0], this.offsetParent[0])) ? this.offsetParent : this.scrollParent, j = /(html|body)/i.test(l[0].tagName), k = h.pageX, i = h.pageY;
	...
	a.ui.plugin.add("draggable", "connectToSortable", {start:function (g, d) {
		var j = a(this).data("draggable"), h = j.options, i = a.extend({}, d, {item:j.element});
		j.sortables = [];
		a(h.connectToSortable).each(function () {
			var b = a.data(this, "sortable");
			if (b && !b.options.disabled) {
				j.sortables.push({instance:b, shouldRevert:b.options.revert});
				b._refreshItems();
				b._trigger("activate", g, i);
			}
		});
	}, stop:function (e, d) {
		var h = a(this).data("draggable"), g = a.extend({}, d, {item:h.element});
		...
		var h = a(this).data("draggable"), g = this;
		a.each(h.sortables, function () {
	...
	a.ui.plugin.add("draggable", "scroll", {start:function () {
		...
		var d = a(this).data("draggable"), h = d.options, g = false;
...
(function (a) {
	...
	a.ui.ddmanager = {current:null, droppables:{"default":[]}, prepareOffsets:function (i, d) {
		var n = a.ui.ddmanager.droppables[i.options.scope] || [], m = d ? d.type : null, k = (i.currentItem || i.element).find(":data(droppable)").andSelf(), l = 0;
	i:
		for (; l < n.length; l++) {
...
(function (c) {
	c.widget("ui.resizable", c.ui.mouse, {widgetEventPrefix:"resize", options:{alsoResize:false, animate:false, animateDuration:"slow", animateEasing:"swing", aspectRatio:false, autoHide:false, containment:false, ghost:false, grid:false, handles:"e,s,se", helper:false, maxHeight:null, maxWidth:null, minHeight:10, minWidth:10, zIndex:1000}, _create:function () {
		...
		if (this.handles.constructor == String) {
			...
			for (var k = 0; k < l.length; k++) {
				var j = c.trim(l[k]), i = c("<div class=\"ui-resizable-handle " + ("ui-resizable-" + j) + "\"></div>");
		...
		var g = this.options, j = this.element.position(), i = this.element;
		this.resizing = true;
		this.documentScroll = {top:c(document).scrollTop(), left:c(document).scrollLeft()};
		if (i.is(".ui-draggable") || /absolute/.test(i.css("position"))) {
			i.css({position:"absolute", top:j.top, left:j.left});
		}
		...
		if (this._helper) {
			var k = this._proportionallyResizeElements, j = k.length && /textarea/i.test(k[0].nodeName);
		...
		var u = this.options, r = this.axis, q = b(s.width) && u.maxWidth && u.maxWidth < s.width, p = b(s.height) && u.maxHeight && u.maxHeight < s.height, o = b(s.width) && u.minWidth && u.minWidth > s.width, n = b(s.height) && u.minHeight && u.minHeight > s.height;
		if (o) {
			s.width = u.minWidth;
		}
		if (n) {
		...
		if (this._proportionallyResizeElements.length) {
			for (var e = this.helper || this.element, g = 0; g < this._proportionallyResizeElements.length; g++) {
				...
				if (!this.borderDif) {
					var i = [j.css("borderTopWidth"), j.css("borderRightWidth"), j.css("borderBottomWidth"), j.css("borderLeftWidth")], h = [j.css("paddingTop"), j.css("paddingRight"), j.css("paddingBottom"), j.css("paddingLeft")];
	...
	c.extend(c.ui.resizable, {version:"1.8.4"});
	c.ui.plugin.add("resizable", "alsoResize", {start:function () {
		...
		e = n.options;
		var m = n.originalSize, l = n.originalPosition, k = {height:n.size.height - m.height || 0, width:n.size.width - m.width || 0, top:n.position.top - l.top || 0, left:n.position.left - l.left || 0}, j = function (f, d) {
			c(f).each(function () {
				var g = c(this), o = c(this).data("resizable-alsoresize"), s = {}, h = d && d.length ? d : g.parents(i.originalElement[0]).length ? ["width", "height"] : ["width", "height", "top", "left"];
		...
		var d = c(this).data("resizable"), e = d.options, f = function (g) {
			c(g).each(function () {
				var h = c(this);
	...
	c.ui.plugin.add("resizable", "animate", {stop:function (e) {
		var i = c(this).data("resizable"), n = i.options, m = i._proportionallyResizeElements, l = m.length && /textarea/i.test(m[0].nodeName), k = l && c.ui.hasScroll(m[0], "left") ? 0 : i.sizeDiff.height;
		...
		i.element.animate(c.extend(l, j && k ? {top:j, left:k} : {}), {duration:n.animateDuration, easing:n.animateEasing, step:function () {
			var d = {width:parseInt(i.element.css("width"), 10), height:parseInt(i.element.css("height"), 10), top:parseInt(i.element.css("top"), 10), left:parseInt(i.element.css("left"), 10)};
	...
	c.ui.plugin.add("resizable", "containment", {start:function () {
		var e = c(this).data("resizable"), i = e.element, n = e.options.containment;
		...
		var i = c(this).data("resizable"), n = i.options, m = i.containerOffset, l = i.position;
	...
	c.ui.plugin.add("resizable", "ghost", {start:function () {
		var d = c(this).data("resizable"), e = d.options, f = d.size;
	...
	c.ui.plugin.add("resizable", "grid", {resize:function () {
		var e = c(this).data("resizable"), i = e.options, n = e.size, m = e.originalSize, l = e.originalPosition, k = e.axis;
	...
	a.widget("ui.sortable", a.ui.mouse, {widgetEventPrefix:"sort", options:{appendTo:"parent", axis:false, connectWith:false, containment:false, cursor:"auto", cursorAt:false, dropOnEmpty:true, forcePlaceholderSize:false, forceHelperSize:false, grid:false, handle:false, helper:"original", items:"> *", opacity:false, placeholder:false, revert:false, scroll:true, scrollSensitivity:20, scrollSpeed:20, scope:"default", tolerance:"intersect", zIndex:1000}, _create:function () {
		...
		if (e) {
			...
			if (this.options.revert) {
				...
				f.reverting = true;
				a(this.helper).animate({left:d.left - this.offset.parent.left - f.margins.left + (this.offsetParent[0] == document.body ? 0 : this.offsetParent[0].scrollLeft), top:d.top - this.offset.parent.top - f.margins.top + (this.offsetParent[0] == document.body ? 0 : this.offsetParent[0].scrollTop)}, parseInt(this.options.revert, 10) || 500, function () {
					f._clear(e);
				});
			} else {
				this._clear(e, d);
...
(function (a) {
	a.widget("ui.accordion", {options:{active:0, animated:"slide", autoHeight:true, clearStyle:false, collapsible:false, event:"click", fillSpace:false, header:"> li > :first-child,> :not(li):even", icons:{header:"ui-icon-triangle-1-e", headerSelected:"ui-icon-triangle-1-s"}, navigation:false, navigationFilter:function () {
		...
		if (!(this.options.disabled || e.altKey || e.ctrlKey)) {
			var c = a.ui.keyCode, j = this.headers.length, i = this.headers.index(e.target), h = false;
			switch (e.keyCode) {
			  case c.RIGHT:
			  case c.DOWN:
	...
	a.extend(a.ui.accordion, {version:"1.8.4", animations:{slide:function (i, c) {
		i = a.extend({easing:"swing", duration:300}, i, c);
		if (i.toHide.size()) {
			if (i.toShow.size()) {
				var n = i.toShow.css("overflow"), l = 0, k = {}, j = {}, m;
				c = i.toShow;
				m = c[0].style.width;
				...
				i.toHide.filter(":hidden").each(i.complete).end().filter(":visible").animate(j, {step:function (b, d) {
					if (d.prop == "height") {
						l = d.end - d.start === 0 ? 0 : (d.now - d.start) / (d.end - d.start);
					}
					i.toShow[0].style[d.prop] = l * k[d.prop].value + k[d.prop].unit;
				}, duration:i.duration, easing:i.easing, complete:function () {
					i.autoHeight || i.toShow.css("height", "");
					i.toShow.css({width:m, overflow:n});
					i.complete();
				}});
			} else {
				i.toHide.animate({height:"hide", paddingTop:"hide", paddingBottom:"hide"}, i);
			}
		} else {
			i.toShow.animate({height:"show", paddingTop:"show", paddingBottom:"show"}, i);
		}
...
(function (a) {
	a.widget("ui.autocomplete", {options:{appendTo:"body", delay:300, minLength:1, position:{my:"left top", at:"left bottom", collision:"none"}, source:null}, _create:function () {
		var d = this, c = this.element[0].ownerDocument;
		this.element.addClass("ui-autocomplete-input").attr("autocomplete", "off").attr({role:"textbox", "aria-autocomplete":"list", "aria-haspopup":"true"}).bind("keydown.autocomplete", function (e) {
			if (!d.options.disabled) {
				...
				switch (e.keyCode) {
				  ...
				  case b.ENTER:
				  case b.NUMPAD_ENTER:
					d.menu.element.is(":visible") && e.preventDefault();
				  case b.TAB:
					if (!d.menu.active) {
						return;
					}
		...
		this.menu = a("<ul></ul>").addClass("ui-autocomplete").appendTo(a(this.options.appendTo || "body", c)[0]).mousedown(function (e) {
			var b = d.menu.element[0];
			e.target === b && setTimeout(function () {
				a(document).one("mousedown", function (g) {
					g.target !== d.element[0] && g.target !== b && !a.ui.contains(b, g.target) && d.close();
				});
			}, 1);
			setTimeout(function () {
				clearTimeout(d.closing);
			}, 13);
		}).menu({focus:function (e, b) {
			b = b.item.data("item.autocomplete");
			false !== d._trigger("focus", null, {item:b}) && /^key/.test(e.originalEvent.type) && d.element.val(b.value);
		}, selected:function (g, e) {
		...
		if (a.isArray(this.options.source)) {
			...
			this.source = function (e, b) {
				b(a.ui.autocomplete.filter(d, e.term));
			};
		} else {
		...
		var d = this.menu.element.empty().zIndex(this.element.zIndex() + 1), f;
...
(function (a) {
	a.widget("ui.menu", {_create:function () {
		...
		this.deactivate();
		if (this.hasScroll()) {
			var j = e.offset().top - this.element.offset().top, i = this.element.attr("scrollTop"), h = this.element.height();
			if (j < 0) {
		...
		if (this.hasScroll()) {
			if (!this.active || this.last()) {
				...
				var e = this.active.offset().top, h = this.element.height(), g = this.element.children("li").filter(function () {
...
(function (b) {
	var e, c = function (a) {
		...
		var h = a.name, g = a.form, f = b([]);
		if (h) {
			f = g ? b(g).find("[name='" + h + "']") : b("[name='" + h + "']", a.ownerDocument).filter(function () {
				return !this.form;
			});
		}
		return f;
	};
	b.widget("ui.button", {options:{text:true, label:null, icons:{primary:null, secondary:null}}, _create:function () {
		...
		if (h.label === null) {
			h.label = this.buttonElement.html();
		}
		...
		this.buttonElement.addClass("ui-button ui-widget ui-state-default ui-corner-all").attr("role", "button").bind("mouseenter.button", function () {
			...
			h.disabled || b(this).removeClass(f);
		...
		if (this.type === "input") {
			...
			if (g.primary || g.secondary) {
				a.addClass("ui-button-text-icon" + (f ? "s" : g.primary ? "-primary" : "-secondary"));
				...
				if (!this.options.text) {
					a.addClass(f ? "ui-button-icons-only" : "ui-button-icon-only").removeClass("ui-button-text-icons ui-button-text-icon-primary ui-button-text-icon-secondary");
...
(function (b, a) {
	b.widget("ui.dialog", {options:{autoOpen:true, buttons:{}, closeOnEscape:true, closeText:"close", dialogClass:"", draggable:true, hide:null, height:"auto", maxHeight:false, maxWidth:false, minHeight:150, minWidth:150, modal:false, position:{my:"center", at:"center", of:window, collision:"fit", using:function (d) {
		var c = b(this).css(d).offset().top;
		...
		var i = this, c = i.options, n = c.title || i.originalTitle || "&#160;", l = b.ui.dialog.getTitleId(i.element), k = (i.uiDialog = b("<div></div>")).appendTo(document.body).hide().addClass("ui-dialog ui-widget ui-widget-content ui-corner-all " + c.dialogClass).css({zIndex:c.zIndex}).attr("tabIndex", -1).css("outline", 0).keydown(function (d) {
			if (c.closeOnEscape && d.keyCode && d.keyCode === b.ui.keyCode.ESCAPE) {
				i.close(d);
				d.preventDefault();
			}
		}).attr({role:"dialog", "aria-labelledby":l}).mousedown(function (d) {
			i.moveToTop(false, d);
		});
		i.element.show().removeAttr("title").addClass("ui-dialog-content ui-widget-content").appendTo(k);
		var m = (i.uiDialogTitlebar = b("<div></div>")).addClass("ui-dialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix").prependTo(k), j = b("<a href=\"#\"></a>").addClass("ui-dialog-titlebar-close ui-corner-all").attr("role", "button").hover(function () {
			...
			if (c.options.modal) {
				...
				b(".ui-dialog").each(function () {
					if (this !== c.uiDialog[0]) {
						f = Math.max(f, b(this).css("z-index"));
					}
				});
				b.ui.dialog.maxZ = f;
			}
		...
		return this._isOpen;
	}, moveToTop:function (e, c) {
		var h = this, g = h.options;
		if (g.modal && !e || !g.stack && !g.modal) {
			return h._trigger("focus", c);
		}
		if (g.zIndex > b.ui.dialog.maxZ) {
			b.ui.dialog.maxZ = g.zIndex;
		}
		if (h.overlay) {
			b.ui.dialog.maxZ += 1;
		...
		if (!this._isOpen) {
			var e = this, c = e.options, f = e.uiDialog;
			...
			f.show(c.show);
			e.moveToTop(true);
			c.modal && f.bind("keypress.ui-dialog", function (h) {
				if (h.keyCode === b.ui.keyCode.TAB) {
					var d = b(":tabbable", this), i = d.filter(":first");
					d = d.filter(":last");
		...
		var c = this, j = false, i = b("<div></div>").addClass("ui-dialog-buttonpane ui-widget-content ui-helper-clearfix"), h = b("<div></div>").addClass("ui-dialog-buttonset").appendTo(i);
		c.uiDialog.find(".ui-dialog-buttonpane").remove();
		typeof e === "object" && e !== null && b.each(e, function () {
			return !(j = true);
		});
		if (j) {
			b.each(e, function (f, d) {
				f = b("<button type=\"button\"></button>").text(f).click(function () {
					d.apply(c.element[0], arguments);
				}).appendTo(h);
		...
		c.uiDialog.draggable({cancel:".ui-dialog-content, .ui-dialog-titlebar-close", handle:".ui-dialog-titlebar", containment:"document", start:function (f, d) {
			h = j.height === "auto" ? "auto" : b(this).height();
			b(this).height(b(this).height()).addClass("ui-dialog-dragging");
			c._trigger("dragStart", f, e(d));
		}, drag:function (f, d) {
			c._trigger("drag", f, e(d));
		}, stop:function (f, d) {
			j.position = [d.position.left - i.scrollLeft(), d.position.top - i.scrollTop()];
		...
		function c(d) {
			return {originalPosition:d.originalPosition, originalSize:d.originalSize, position:d.position, size:d.size};
		}
		e = e === a ? this.options.resizable : e;
		var j = this, i = j.options, h = j.uiDialog.css("position");
		e = typeof e === "string" ? e : "n,e,s,w,se,sw,ne,nw";
		j.uiDialog.resizable({cancel:".ui-dialog-content", containment:"document", alsoResize:j.element, maxWidth:i.maxWidth, maxHeight:i.maxHeight, minWidth:i.minWidth, minHeight:j._minHeight(), handles:e, start:function (f, d) {
			b(this).addClass("ui-dialog-resizing");
			j._trigger("resizeStart", f, c(d));
		}, resize:function (f, d) {
			j._trigger("resize", f, c(d));
		}, stop:function (f, d) {
		...
		if (e) {
			if (typeof e === "string" || typeof e === "object" && "0" in e) {
				c = e.split ? e.split(" ") : [e[0], e[1]];
				if (c.length === 1) {
					c[1] = c[0];
				}
				b.each(["left", "top"], function (d, f) {
					if (+c[d] === c[d]) {
						h[d] = c[d];
						c[d] = f;
					}
		...
		var l = this, j = l.uiDialog, i = j.is(":data(resizable)"), k = false;
		switch (h) {
		  case "beforeclose":
			h = "beforeClose";
			break;
		  ...
		  case "closeText":
			...
			break;
		  case "dialogClass":
			j.removeClass(l.options.dialogClass).addClass("ui-dialog ui-widget ui-widget-content ui-corner-all " + c);
			break;
		  case "disabled":
			c ? j.addClass("ui-dialog-disabled") : j.removeClass("ui-dialog-disabled");
			break;
		  case "draggable":
			c ? l._makeDraggable() : j.draggable("destroy");
			break;
		  case "height":
			k = true;
			break;
		  case "maxHeight":
			i && j.resizable("option", "maxHeight", c);
			k = true;
			break;
		  case "maxWidth":
			i && j.resizable("option", "maxWidth", c);
			k = true;
			break;
		  case "minHeight":
			i && j.resizable("option", "minHeight", c);
			k = true;
			break;
		  case "minWidth":
			i && j.resizable("option", "minWidth", c);
			k = true;
			break;
		  case "position":
			l._position(c);
	...
	b.extend(b.ui.dialog, {version:"1.8.4", uuid:0, maxZ:0, getTitleId:function (c) {
		c = c.attr("id");
		if (!c) {
			this.uuid += 1;
			c = this.uuid;
	...
	b.extend(b.ui.dialog.overlay, {instances:[], oldInstances:[], maxZ:0, events:b.map("focus,mousedown,mouseup,keydown,keypress,click".split(","), function (c) {
		...
		if (this.instances.length === 0) {
			setTimeout(function () {
				b.ui.dialog.overlay.instances.length && b(document).bind(b.ui.dialog.overlay.events, function (e) {
		...
		this.oldInstances.push(this.instances.splice(b.inArray(d, this.instances), 1)[0]);
...
(function (g, f) {
	...
	g.widget("ui.tabs", {options:{add:null, ajaxOptions:null, cache:false, cookie:null, collapsible:false, disable:null, disabled:[], enable:null, event:"click", fx:null, idPrefix:"ui-tabs-", load:null, panelTemplate:"<div></div>", remove:null, select:null, show:null, spinner:"<em>Loading&#8230;</em>", tabTemplate:"<li><a href='#{href}'><span>#{label}</span></a></li>"}, _create:function () {
		...
		return d.title && d.title.replace(/\s/g, "_").replace(/[^A-Za-z0-9\-_:\.]/g, "") || this.options.idPrefix + e();
		...
		function w(h, i) {
			h.css("display", "");
			!g.support.opacity && i.opacity && h[0].style.removeAttribute("filter");
		}
		...
		this.anchors.each(function (k, n) {
			var i = g(n).attr("href"), h = i.split("#")[0], m;
			if (h && (h === location.toString().split("#")[0] || (m = g("base")[0]) && h === m.href)) {
				i = n.hash;
				n.href = i;
			}
			if (v.test(i)) {
				y.panels = y.panels.add(y._sanitizeSelector(i));
			} else {
				if (i !== "#") {
					g.data(n, "href.tabs", i);
					g.data(n, "load.tabs", i.replace(/#.*$/, ""));
					i = y._tabId(n);
					n.href = "#" + i;
					n = g("#" + i);
					if (!n.length) {
						n = g(x.panelTemplate).attr("id", i).addClass("ui-tabs-panel ui-widget-content ui-corner-bottom").insertAfter(y.panels[k - 1] || y.list);
		...
		if (z) {
			...
			if (x.selected === f) {
				location.hash && this.anchors.each(function (h, i) {
				...
				if (typeof x.selected !== "number" && x.cookie) {
					x.selected = parseInt(y._cookie(), 10);
				}
			...
			if (x.selected >= 0 && this.anchors.length) {
				this.panels.eq(x.selected).removeClass("ui-tabs-hide");
				this.lis.eq(x.selected).addClass("ui-tabs-selected ui-state-active");
				y.element.queue("tabs", function () {
					y._trigger("show", null, y._ui(y.anchors[x.selected], y.panels[x.selected]));
				});
				this.load(x.selected);
			}
			g(window).bind("unload", function () {
				y.lis.add(y.anchors).unbind(".tabs");
		...
		z = 0;
		for (var u; u = this.lis[z]; z++) {
			g(u)[g.inArray(z, x.disabled) != -1 && !g(u).hasClass("ui-tabs-selected") ? "addClass" : "removeClass"]("ui-state-disabled");
		}
		x.cache === false && this.anchors.removeData("cache.tabs");
		this.lis.add(this.anchors).unbind(".tabs");
		if (x.event !== "mouseover") {
			var q = function (h, i) {
				i.is(":not(.ui-state-disabled)") && i.addClass("ui-state-" + h);
			}, l = function (h, i) {
				i.removeClass("ui-state-" + h);
			};
		...
		var d = j ? function (h, i) {
			g(h).closest("li").addClass("ui-tabs-selected ui-state-active");
			i.hide().removeClass("ui-tabs-hide").animate(j, j.duration || "normal", function () {
				w(i, j);
				y._trigger("show", null, y._ui(h, i[0]));
			});
		} : function (h, i) {
			g(h).closest("li").addClass("ui-tabs-selected ui-state-active");
			...
			i.animate(p, p.duration || "normal", function () {
				y.lis.removeClass("ui-tabs-selected ui-state-active");
		...
		this.anchors.bind(x.event + ".tabs", function () {
			var k = this, m = g(k).closest("li"), i = y.panels.filter(":not(.ui-tabs-hide)"), h = g(y._sanitizeSelector(k.hash));
			if (m.hasClass("ui-tabs-selected") && !x.collapsible || m.hasClass("ui-state-disabled") || m.hasClass("ui-state-processing") || y._trigger("select", null, y._ui(this, h[0])) === false) {
				this.blur();
				return false;
			}
			...
			x.cookie && y._cookie(x.selected, x.cookie);
			if (h.length) {
				i.length && y.element.queue("tabs", function () {
					A(k, i);
				});
				y.element.queue("tabs", function () {
					d(k, h);
		...
		m = g(l.tabTemplate.replace(/#\{href\}/g, j).replace(/#\{label\}/g, m));
		j = !j.indexOf("#") ? j.replace("#", "") : this._tabId(g("a", m)[0]);
		m.addClass("ui-state-default ui-corner-top").data("destroy.tabs", true);
		var k = g("#" + j);
		k.length || (k = g(l.panelTemplate).attr("id", j).data("destroy.tabs", true));
		...
		if (d.hasClass("ui-tabs-selected") && this.anchors.length > 1) {
		...
		j = this._getIndex(j);
		var m = this, d = this.options, n = this.anchors.eq(j)[0], l = g.data(n, "load.tabs");
		this.abort();
		if (!l || this.element.queue("tabs").length !== 0 && g.data(n, "cache.tabs")) {
			...
			this.xhr = g.ajax(g.extend({}, d.ajaxOptions, {url:l, success:function (i, o) {
				g(m._sanitizeSelector(n.hash)).html(i);
				m._cleanup();
				d.cache && g.data(n, "cache.tabs", true);
				m._trigger("load", null, m._ui(m.anchors[j], m.panels[j]));
				try {
					d.ajaxOptions.success(i, o);
				}
				catch (h) {
				}
			}, error:function (i, o) {
	...
	g.extend(g.ui.tabs, {version:"1.8.4"});
	g.extend(g.ui.tabs.prototype, {rotation:null, rotate:function (i, k) {
		var d = this, l = this.options, j = d._rotate || (d._rotate = function (h) {
		...
		k = d._unrotate || (d._unrotate = !k ? function (h) {
			h.clientX && d.rotate(null);
		} : function () {
			t = l.selected;
			j();
		});
...
(function (d, G) {
	...
	d.extend(L.prototype, {markerClassName:"hasDatepicker", log:function () {
		...
		var advance = showAdvance  ? ("<button type=\"button\" class=\"ui-datepicker-close ui-state-default ui-priority-primary ui-corner-all\" onclick=\"advButton(" + $(a).attr("id") + ")\">高级模式</button>") : "";
...
jQuery.effects || function (q, i) {
	...
	function a(k, j) {
		...
		do {
			f = q.curCSS(k, j);
			if (f != "" && f != "transparent" || q.nodeName(k, "body")) {
				break;
			}
			j = "backgroundColor";
	...
	function c(k) {
		...
		for (j in k) {
			f = k[j];
			if (f == null || q.isFunction(f) || j in v || /scrollbar/.test(j) || !/color/i.test(j) && isNaN(parseFloat(f))) {
				delete k[j];
			}
		}
		return k;
	...
	function h(l, j, f, k) {
		...
		if (typeof j == "number" || q.fx.speeds[j]) {
			k = f;
			f = j;
			j = {};
		}
	...
	q.effects = {};
	q.each(["backgroundColor", "borderBottomColor", "borderLeftColor", "borderRightColor", "borderTopColor", "color", "outlineColor"], function (j, f) {
		q.fx.step[f] = function (k) {
			if (!k.colorInit) {
				k.start = a(k.elem, f);
				k.end = g(k.end);
				k.colorInit = true;
	...
	q.effects.animateClass = function (l, j, f, k) {
		if (q.isFunction(f)) {
			k = f;
			f = null;
		}
		return this.each(function () {
			var r = q(this), o = r.attr("style") || " ", n = c(d.call(this)), p, m = r.attr("className");
			q.each(b, function (s, w) {
				l[w] && r[w + "Class"](l[w]);
			});
			p = c(d.call(this));
			r.attr("className", m);
			r.animate(u(n, p), j, f, function () {
				q.each(b, function (s, w) {
					l[w] && r[w + "Class"](l[w]);
				});
	...
	q.fn.extend({_addClass:q.fn.addClass, addClass:function (l, j, f, k) {
		return j ? q.effects.animateClass.apply(this, [{add:l}, j, f, k]) : this._addClass(l);
	}, _removeClass:q.fn.removeClass, removeClass:function (l, j, f, k) {
		return j ? q.effects.animateClass.apply(this, [{remove:l}, j, f, k]) : this._removeClass(l);
	}, _toggleClass:q.fn.toggleClass, toggleClass:function (m, j, f, l, k) {
		return typeof j == "boolean" || j === i ? f ? q.effects.animateClass.apply(this, [j ? {add:m} : {remove:m}, f, l, k]) : this._toggleClass(m, j) : q.effects.animateClass.apply(this, [{toggle:m}, j, f, l]);
	}, switchClass:function (m, j, f, l, k) {
		return q.effects.animateClass.apply(this, [{add:j, remove:m}, f, l, k]);
	}});
	q.extend(q.effects, {version:"1.8.4", save:function (k, j) {
		...
		if (k.css("position") == "static") {
			...
			q.extend(j, {position:k.css("position"), zIndex:k.css("z-index")});
			q.each(["top", "left", "bottom", "right"], function (m, l) {
				j[l] = k.css(l);
	...
	}, _hide:q.fn.hide, hide:function (j) {
		if (!j || typeof j == "number" || q.fx.speeds[j]) {
	...
	}, __toggle:q.fn.toggle, toggle:function (j) {
		if (!j || typeof j == "number" || q.fx.speeds[j] || typeof j == "boolean" || q.isFunction(j)) {
	...
	q.extend(q.easing, {def:"easeOutQuad", swing:function (m, j, f, l, k) {
		return q.easing[q.easing.def](m, j, f, l, k);
		...
		return l - q.easing.easeOutBounce(m, k - j, 0, l, k) + f;
		...
		if (j < k / 2) {
			return q.easing.easeInBounce(m, j * 2, 0, l, k) * 0.5 + f;
		}
...
(function (a) {
	a.effects.bounce = function (c) {
		return this.queue(function () {
			var w = a(this), e = ["position", "top", "left"], q = a.effects.setMode(w, c.options.mode || "effect"), u = c.options.direction || "up", v = c.options.distance || 20, b = c.options.times || 5, p = c.duration || 250;
...
(function (a) {
	a.effects.clip = function (b) {
		return this.queue(function () {
			var e = a(this), j = ["position", "top", "left", "height", "width"], m = a.effects.setMode(e, b.options.mode || "hide"), o = b.options.direction || "vertical";
			a.effects.save(e, j);
			e.show();
			...
			n.animate(k, {queue:false, duration:b.duration, easing:b.options.easing, complete:function () {
				m == "hide" && e.hide();
...
(function (a) {
	a.effects.highlight = function (b) {
		return this.queue(function () {
			var c = a(this), h = ["backgroundImage", "backgroundColor", "opacity"], i = a.effects.setMode(c, b.options.mode || "show"), g = {backgroundColor:c.css("backgroundColor")};
			if (i == "hide") {
				g.opacity = 0;
			}
			a.effects.save(c, h);
...
(function (a) {
	a.effects.transfer = function (b) {
		return this.queue(function () {
			var e = a(this), i = a(b.options.to), h = i.offset();
			i = {top:h.top, left:h.left, height:i.innerHeight(), width:i.innerWidth()};
			h = e.offset();
			var g = a("<div class=\"ui-effects-transfer\"></div>").appendTo(document.body).addClass(b.options.className).css({top:h.top, left:h.left, height:e.innerHeight(), width:e.innerWidth(), position:"absolute"}).animate(i, b.duration, b.options.easing, function () {
				g.remove();
				b.callback && b.callback.apply(e[0], arguments);
				e.dequeue();
			});
								...
								var oldHtml = "";     //原有的日历控件的html
								var htmlValue = "";   //日历控件高级模式的html
								var intArr = new Array();
								var quarterArr = new Array();
								
								//点击高级模式的事件
								function advButton(input) {
					        	var height = $("#ui-datepicker-div").css("height");
					     			
					     			oldHtml = $("#ui-datepicker-div").html();
					        	$("#ui-datepicker-div").empty();
					        	
					        	htmlValue = "<div class=\"inner\" align=\"center\" >"+
					        	...
					        	"<td align=\"right\">年：</td>"+
					        	"<td>"+
					        	"<select id=\"yearType\" style=\"width:70px;height:22px;\">"+
					        	"<option value=\"1\" selected=\"selected\">增量值</option>"+
					        	"<option value=\"2\">固定值</option>"+
					        	"</select>"+
					        	"</td>"+
					        	"<td>&nbsp;<input type=\"text\" id=\"yearValue\" value=\"0\" style=\"width:60px;height:22px;\"/></td>"+
					        	"</tr>"+
					        	"<tr>"+
					        	"<td align=\"right\">季度：</td>"+
					        	"<td>"+
					        	"<select id=\"quarterType\" style=\"width:70px;height:22px;\">"+
					        	"<option value=\"1\" selected=\"selected\">不限</option>"+
					        	"<option value=\"2\" >增量值</option>"+
					        	"<option value=\"3\">固定值</option>"+
					        	"</select>"+
					        	"</td>"+
					        	"<td>&nbsp;<input type=\"text\" id=\"quarterValue\" value=\"\" style=\"width:60px;height:22px;\"/></td>"+
					        	"</tr>"+
					        	"<tr>"+
					        	"<td align=\"right\">月：</td>"+
					        	"<td>"+
					        	"<select id=\"monthType\" style=\"width:70px;height:22px;\">"+
					        	"<option value=\"1\" selected=\"selected\" >增量值</option>"+
					        	"<option value=\"2\">固定值</option>"+
					        	"<option value=\"3\">季度第几个</option>"+
					        	"</select>"+
					        	"</td>"+
					        	"<td>&nbsp;<input type=\"text\" id=\"monthValue\" value=\"0\" style=\"width:60px;height:22px;\"/></td>"+
					        	"</tr>"+
					        	"<tr>"+
					        	"<td align=\"right\">日：</td>"+
					        	"<td>"+
					        	"<select id=\"dayType\" style=\"width:70px;height:22px;\">"+
					        	"<option value=\"1\" selected=\"selected\" >增量值</option>"+
					        	"<option value=\"2\">固定值</option>"+
					        	"<option value=\"3\">倒数第几日</option>"+
					        	"</select>"+
					        	"</td>"+
					        	"<td>&nbsp;<input id=\"dayValue\" type=\"text\" value=\"0\" style=\"width:60px;height:22px;\"/></td>"+
					        	"</tr>"+
					        	"</table>"+
					        	"</div>"+
					        	...
					        	"<td align=\"right\">周：</td>"+
					        	"<td>"+
					        	"<select id=\"weekType\" style=\"width:70px;height:22px;\">"+
					        	"<option value=\"1\" selected=\"selected\">增量值</option>"+
					        	"</select>"+
					        	"</td>"+
					        	"<td>&nbsp;<input id=\"weekValue\" type=\"text\" value=\"0\" style=\"width:60px;height:22px;\"/></td>"+
					        	"</tr>"+
					        	"<tr>"+
					        	"<td align=\"right\">日：</td>"+
					        	"<td><select id=\"weekDayType\" style=\"width:70px;height:22px;\">"+
					        	"<option value=\"1\" selected=\"selected\">周的第几日</option>"+
					        	"</select>"+
					        	"</td>"+
					        	"<td>&nbsp;<input id=\"weekDayValue\" type=\"text\" value=\"0\" style=\"width:60px;height:22px;\"/></td>"+
					        	"</tr>"+
					        	"</table>"+
					        	"</div>"+
					        	"<div class=\"bottom\">"+
					        	"<div class=\"bom1\"><a href=\"javascript:void(0);\" onclick=\"normal()\">常规模式</a></div>"+
					        	"<div class=\"bom2\"><input type=\"button\" onclick=\"okButton('"+ $(input).attr("id") +"')\" value=\"确定\"/>&nbsp;<input type=\"button\" onclick=\"myCancle()\" value=\"取消\"/></div>"+
					        	"</div></div>";
					        	
					        	$("#ui-datepicker-div").attr("align","center");
					        	$("#ui-datepicker-div").html(htmlValue);
					        	$("#ui-datepicker-div").css("height",height);
					      }  
        ...
        function okButton(inputid){
      		...
      		var intYear=today.getFullYear();
					...
					var yearValue = parseInt($("#yearValue").val());
					var quarterType = $("#quarterType").val();
					var quarterValue = ($("#quarterValue").val() == null || $("#quarterValue").val() == "") ? parseInt(0) : parseInt($("#quarterValue").val());
					var monthType = $("#monthType").val();
					var monthValue = parseInt($("#monthValue").val());
					var dayType = $("#dayType").val();
					var dayValue = parseInt($("#dayValue").val());
					var weekType = $("#weekType").val();
					var weekValue = parseInt($("#weekValue").val());
					var weekDayType = $("#weekDayType").val();
					var weekDayValue = parseInt($("#weekDayValue").val());
					...
					$("#" + inputid).val(rsYear + "-" + rsMonth + "-" + rsDay);
      	...
      	function normal(){
      		var height = $("#ui-datepicker-div").css("height");
					$("#ui-datepicker-div").empty();
      		$("#ui-datepicker-div").html(oldHtml);
